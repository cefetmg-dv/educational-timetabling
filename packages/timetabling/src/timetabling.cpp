...
Problem load_problem(const std::string& raw_data) {
    ...
    for (const auto& teacher : data["teachers"]) {
        id_type teacher_id = teacher["id"].get<id_type>();
        std::set<id_type> teacher_events;
        for (const auto& event : data["events"]) {
            id_type event_id = event["id"].get<id_type>();
            if (event["teacher"].get<id_type>() == teacher_id) {
    ...
    for (const auto& clazz : data["classes"]) {
        id_type class_id = clazz["id"].get<id_type>();
        std::set<id_type> class_events;
        for (const auto& event : data["events"]) {
            id_type event_id = event["id"].get<id_type>();
            if (event["class"].get<id_type>() == class_id) {
    ...
    for (const auto& rule : data["precedence_rules"]) {
        id_type event1 = rule["event"].get<id_type>();
        for (const auto& event2 : rule["dependent_events"])
        problem.precedence.emplace(event1, event2.get<id_type>());
    }

    // Events must be assigned to compatible rooms (compatibility constraints)
    for (const auto& event : data["events"]) {
        id_type event_id = event["id"].get<id_type>();
        id_type class_id = event["class"].get<id_type>();
        id_type subject_id = event["subject"].get<id_type>();
        for (const auto& room : data["classes"][class_id]["subjects"][subject_id]["rooms"]) {
            id_type room_id = room.get<id_type>();
    ...
    for (const auto& room : data["rooms"]) {
        id_type room_id = room["id"].get<id_type>();
        for (const auto& unavailability : room["unavailability"]) {
            id_type timeslot_id = unavailability.get<id_type>();
